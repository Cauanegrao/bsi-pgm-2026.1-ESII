...
O teste de integração captura falhas que os testes de unidade não conseguem detetar, como erros de importação entre ficheiros, incompatibilidades de tipos ao passar dados reais entre camadas e falhas de comunicação direta entre o ServicoEmprestimo e os módulos de infraestrutura (repositórios e serviços). Ele garante que a "fiação" do sistema está correta. Por outro lado, o teste de integração não captura de forma cirúrgica o local exato de um erro de lógica interna profunda. Se um cálculo de multa falhar, o teste de integração indica que o fluxo falhou, mas um teste de unidade isolado captura muito melhor a causa exata na função específica, pois avalia o comportamento da regra de negócio sem interferência do estado de outros componentes.
